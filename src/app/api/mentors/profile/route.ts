      ...
      ...(q
        ? {
            OR: [
              { username: { contains: q } },
              { name: { contains: q } },
              { MentorProfile: { bio: { contains: q } } },
              { MentorProfile: { tags: { some: { tag: { contains: q } } } } },
            ],
          }
        : {}),
    },
...
export async function POST(req: Request) {
  ...
  const { bio, tags, name, age, experience } = await req.json()
  if (!Array.isArray(tags) || tags.some((t) => typeof t !== 'string')) {
    return new Response('Tags must be an array of strings', { status: 400 })
  }
  // Only update name on user
  await db.user.update({
    where: { id: session.user.id },
    data: { name },
  })
  const latestApp = await db.mentorApplication.findFirst({
  ...
  // Return updated user with profile and latest approved application
  const user = await db.user.findUnique({
    where: { id: session.user.id },
    include: {
      MentorProfile: { include: { tags: true } },
